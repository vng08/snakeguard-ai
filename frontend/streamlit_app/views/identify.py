...
def render_identify_page():
    ...
    uploaded_file = st.file_uploader("Chọn ảnh", type=["jpg", "jpeg", "png", "webp", "bmp", "tiff"], key="identify_upload")

    if uploaded_file:
        st.caption(f"📎 Đã chọn: {uploaded_file.name}")

        if st.button("🔍 Đang nhận diện..." if running else "🔍 Nhận diện", type="primary", width="stretch", disabled=running, key="identify_submit"):
...
def _run_identification():
    ...
    try:
        ...
        st.session_state.identify_result = {"result": result, "image": pending["image"]}

    except requests.RequestException:
        st.session_state.identify_result = {"error": ERROR_MESSAGE}
